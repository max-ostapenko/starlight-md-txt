...
`starlight-md-txt` is a community Starlight plugin to expose your documentation pages as raw, agent-friendly Markdown (e.g. `/getting-started.md`).
    ...
    starlight({
      ...
      plugins: [
        starlightMdTxt({
          ...
          includeDrafts: false,
          format: '.md', // '.md' (default) or '.md.txt'
        }),
      ],
    }),
  ],
...
Now, when you run your build, all of your documentation pages will have a corresponding `.md` file (e.g., `dist/getting-started.md`) alongside the standard HTML pages.
